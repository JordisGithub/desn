...
async function processImage(file) {
  ...
  const name = rel.replace(/[^a-z0-9]+/gi, '-').replace(/^-|-$/g, '').toLowerCase();
  ...
  for (const w of sizes) {
    ...
    try {
      ...
      meta.variants[w] = { webp: `./optimized/${path.basename(outWebp)}`, jpg: `./optimized/${path.basename(outJpg)}` };
    } catch (e) {
  ...
  if (!meta.variants[1600]) {
    ...
    await image.resize(1600).webp({ quality: 75 }).toFile(outWebp).catch(()=>{});
...
(async function main(){
  ...
  await fs.promises.writeFile(META_FILE, JSON.stringify(metas, null, 2), 'utf8');
