...
- `public/robots.txt` now disallows private dashboard/auth paths and references sitemap.
- Generate sitemap after adding/removing routes:
  ```bash
  node scripts/generate-sitemap.cjs
  ```
  Deploy updated `public/sitemap.xml` via normal build.
- After deployment submit sitemap: `https://desnepal.org/sitemap.xml` to Google Search Console.
- Added canonical `<link>` tag in `index.html` pointing to `https://desnepal.org/`.
- Use verification script post-cutover:
  ```bash
